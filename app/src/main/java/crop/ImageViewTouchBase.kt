...
abstract class ImageViewTouchBase : AppCompatImageView, IDisposable {
        ...
        if (drawable != null) {
            if (changed || mScaleTypeChanged || mBitmapChanged) {
                ...
                if (LOG_ENABLED) {
                    Log.d(
                        LOG_TAG,
                        "old matrix scale: $old_matrix_scale"
                    )
                    Log.d(
                        LOG_TAG,
                        "new matrix scale: $new_matrix_scale"
                    )
                    Log.d(
                        LOG_TAG,
                        "old min scale: $old_min_scale"
                    )
                    Log.d(
                        LOG_TAG,
                        "old scale: $old_scale"
                ...
                if (mBitmapChanged || mScaleTypeChanged) {
                    ...
                    if (LOG_ENABLED) {
                        Log.d(
                            LOG_TAG,
                            "old min scale: $old_default_scale"
                        )
                        Log.d(
                            LOG_TAG,
                            "old scale: $old_scale"
    ...
    fun setImageBitmap(
        ...
        if (bitmap != null) setImageDrawable(
    ...
    protected fun getProperBaseMatrix2(bitmap: Drawable, matrix: Matrix) {
        ...
        matrix.postTranslate(
            (viewWidth - w * scale) / 2.0f, (viewHeight - h
                    * scale) / 2.0f
        )
    }
    ...
    protected fun updateRect(bitmapRect: RectF?, scrollRect: RectF) {
        ...
        ) scrollRect.top = (0 - bitmapRect.top)
        if (bitmapRect.bottom + scrollRect.top <= mThisHeight - 0
            && bitmapRect.top < 0
        ) scrollRect.top = (mThisHeight - 0 - bitmapRect.bottom)
    ...
    protected fun scrollBy(
        ...
        mHandler.post(object : Runnable {
            ...
            override fun run() {
                ...
                val x: Double = mEasing.easeOut(currentMs, 0.0, dx, durationMs)
                val y: Double = mEasing.easeOut(currentMs, 0.0, dy, durationMs)
                ...
                if (currentMs < durationMs) {
                    ...
                    if (centerRect.left != 0f || centerRect.top != 0f) scrollBy(
                        centerRect.left,
                        centerRect.top
                    )
                }
