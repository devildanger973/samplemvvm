...
class ImageViewTouch(context: Context?, attrs: AttributeSet?) :
    ...
    protected var mScaleListener: OnScaleGestureListener? = null
    ...
    protected val gestureListener: GestureDetector.OnGestureListener
        protected get() = GestureListener()
    protected val scaleListener: OnScaleGestureListener
        protected get() = ScaleListener()
    ...
    /*override fun onZoomAnimationCompleted(scale: Float) {
        if (LOG_ENABLED) {
            Log.d(
                LOG_TAG, "onZoomAnimationCompleted. scale: " + scale
                        + ", minZoom: " + minScale
            )
        }
        if (scale < minScale) {
    ...
    fun canScroll(direction: Int): Boolean {
        ...
        val bitmapScrollRectDelta = Math.abs(
            bitmapRect.left
                    - mScrollRect.left
        ).toDouble()
    ...
    inner class GestureListener : SimpleOnGestureListener() {
        ...
        override fun onDoubleTap(e: MotionEvent): Boolean {
            Log.i(
                LOG_TAG, "onDoubleTap. double tap enabled? "
                        + doubleTapEnabled
            )
            if (doubleTapEnabled) {
                mUserScaled = true
